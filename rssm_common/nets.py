...
class EnsembleRSSM(rssm_common.Module):
  ...
  def observe(self, embed, action, is_first, state=None):
    ...
    post, prior = rssm_common.static_scan(
  ...
  @tf.function
  def observe_1(self, embed, action, is_first, state=None):
    swap = lambda x: tf.transpose(x, [1, 0] + list(range(2, len(x.shape))))
    if state is None:
      state = self.initial(tf.shape(action)[0])
    post, prior = rssm_common.static_scan(
        lambda prev, inputs: self.obs_step_1(prev[0], *inputs),
        (swap(action), swap(embed), swap(is_first)), (state, state))
    post = {k: swap(v) for k, v in post.items()}
    prior = {k: swap(v) for k, v in prior.items()}
    return post, prior
  ...
  @tf.function
  def observe_2(self, embed, action, is_first, state=None):
    swap = lambda x: tf.transpose(x, [1, 0] + list(range(2, len(x.shape))))
    if state is None:
      state = self.initial(tf.shape(action)[0])
    post, prior = rssm_common.static_scan(
        lambda prev, inputs: self.obs_step_2(prev[0], *inputs),
        (swap(action), swap(embed), swap(is_first)), (state, state))
    post = {k: swap(v) for k, v in post.items()}
    prior = {k: swap(v) for k, v in prior.items()}
    return post, prior
  ...
  def imagine(self, action, state=None):
    ...
    action = swap(action)
    prior = rssm_common.static_scan(self.img_step, action, state)
    prior = {k: swap(v) for k, v in prior.items()}
    return prior

  @tf.function
  def imagine_1(self, action, state=None):
    swap = lambda x: tf.transpose(x, [1, 0] + list(range(2, len(x.shape))))
    if state is None:
      state = self.initial(tf.shape(action)[0])
    assert isinstance(state, dict), state
    action = swap(action)
    prior = rssm_common.static_scan(self.img_step_1, action, state)
    prior = {k: swap(v) for k, v in prior.items()}
    return prior

  @tf.function
  def imagine_2(self, action, state=None):
    swap = lambda x: tf.transpose(x, [1, 0] + list(range(2, len(x.shape))))
    if state is None:
      state = self.initial(tf.shape(action)[0])
    assert isinstance(state, dict), state
    action = swap(action)
    prior = rssm_common.static_scan(self.img_step_2, action, state)
    prior = {k: swap(v) for k, v in prior.items()}
    return prior
  ...
  def get_dist(self, state, ensemble=False):
    ...
    if self._discrete:
      ...
      #tf.print("dist kl logit: ",logit.shape)
      dist = tfd.Independent(rssm_common.OneHotDist(logit), 1)
    else:
  ...
  def obs_step(self, prev_state, prev_action, embed, is_first, sample=True):
    ...
    #tf.print("about to do obs sample")
    stoch = dist.sample() if sample else dist.mode()
    post = {'stoch': stoch, 'deter': prior['deter'], **stats}
    return post, prior

  @tf.function
  def obs_step_1(self, prev_state, prev_action, embed, is_first, sample=True):
    # if is_first.any():
    prev_state, prev_action = tf.nest.map_structure(
        lambda x: tf.einsum(
            'b,b...->b...', 1.0 - is_first.astype(x.dtype), x),
        (prev_state, prev_action))
    prior = self.img_step_1(prev_state, prev_action, sample)
    x = tf.concat([prior['deter'], embed], -1)
    x = self.get('obs_out', tfkl.Dense, self._hidden)(x)
    x = self.get('obs_out_norm', NormLayer, self._norm)(x)
    x = self._act(x)
  ...
  @tf.function
  def obs_step_2(self, prev_state, prev_action, embed, is_first, sample=True):
    # if is_first.any():
    prev_state, prev_action = tf.nest.map_structure(
        lambda x: tf.einsum(
            'b,b...->b...', 1.0 - is_first.astype(x.dtype), x),
        (prev_state, prev_action))
    prior = self.img_step_2(prev_state, prev_action, sample)
    x = tf.concat([prior['deter'], embed], -1)
    x = self.get('obs_out', tfkl.Dense, self._hidden)(x)
    x = self.get('obs_out_norm', NormLayer, self._norm)(x)
    x = self._act(x)
  ...
  def img_step(self, prev_state, prev_action, sample=True):
    ...
    dist = self.get_dist(stats)
    #tf.print("about to do img sample")
    stoch = dist.sample() if sample else dist.mode()
    prior = {'stoch': stoch, 'deter': deter, **stats}
    return prior

  @tf.function
  def img_step_1(self, prev_state, prev_action, sample=True):
    prev_stoch = self._cast(prev_state['stoch'])
    prev_action = self._cast(prev_action)
    if self._discrete:
      shape = prev_stoch.shape[:-2] + [self._stoch * self._discrete]
  ...
  @tf.function
  def img_step_2(self, prev_state, prev_action, sample=True):
    prev_stoch = self._cast(prev_state['stoch'])
    prev_action = self._cast(prev_action)
    if self._discrete:
      shape = prev_stoch.shape[:-2] + [self._stoch * self._discrete]
  ...
  def _suff_stats_ensemble(self, inp):
    ...
    for k in range(self._ensemble):
      ...
      stat = self._suff_stats_layer(f'img_dist_{k}', x)
      stats.append(stat)
    stats = {
  ...
  def kl_loss(self, post, prior, forward, balance, free, free_avg):
    ...
    #tf.print('kl loss: ',loss)
    #tf.print('kl value shape: ',value.shape)
    #tf.print('kl value: ',value)
    return loss, value


class Encoder(rssm_common.Module):
...
class Decoder(rssm_common.Module):
  ...
  def __call__(self, features):
    ...
    #print('dec input shape ',features.shape) #(16, 28, 1624)
    outputs = {}
    if self.cnn_keys:
  ...
  def _mlp(self, features):
    ...
    x = features
    for i, width in enumerate(self._mlp_layers):
      #print('dec inner input shape ',x.shape)
      x = self.get(f'dense{i}', tfkl.Dense, width)(x)
      x = self.get(f'densenorm{i}', NormLayer, self._norm)(x)
      x = self._act(x)
      #print('dec inner out shape ',x.shape)
    dists = {}
    for key, shape in shapes.items():
      #if key == 'image':
        #print('dec output shape: ',x.shape) #(16, 28, 400)
        #print('dist layer dec shape: ',shape)
      dists[key] = self.get(f'dense_{key}', DistLayer, shape)(x)
    return dists


class MLP(rssm_common.Module):
...
class DistLayer(rssm_common.Module):
  ...
  def __call__(self, inputs):
    ...
    if self._dist == 'mse':
      ...
      #tf.print("len of dist param: ",len(self._shape))
    ...
    if self._dist == 'tanh_normal':
      ...
      dist = tfd.TransformedDistribution(dist, rssm_common.TanhBijector())
      dist = tfd.Independent(dist, len(self._shape))
      return rssm_common.SampleDist(dist)
    if self._dist == 'trunc_normal':
      std = 2 * tf.nn.sigmoid((std + self._init_std) / 2) + self._min_std
      dist = rssm_common.TruncNormalDist(tf.tanh(out), std, -1, 1)
      return tfd.Independent(dist, 1)
    if self._dist == 'onehot':
      return rssm_common.OneHotDist(out)
    raise NotImplementedError(self._dist)


class NormLayer(rssm_common.Module):
